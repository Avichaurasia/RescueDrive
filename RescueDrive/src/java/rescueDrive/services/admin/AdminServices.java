...
public class AdminServices {
    ...
    public UserBean getInfoUserByID(int userId) {
        UserBean obj1 = null;
        try (Connection conn = ConnectDB.connect();) {
            PreparedStatement pstmt = conn.prepareStatement("select * from usercontactmaster uc inner join usermaster um on uc.user_id = um.user_id inner join countrymaster cm on uc.country_id = cm.country_id inner join statemaster sm on uc.state_id = sm.state_id inner join citymaster cim on cim.city_id = uc.city_id and um.user_id=?");
            pstmt.setInt(1, userId);
