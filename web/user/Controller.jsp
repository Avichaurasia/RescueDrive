    ...
    if (request.getParameter("sbtnDelete") != null) {
        ...
        if (files != null && files.length > 0) {
            for (int k = 0; k < files.length; k++) {
                File f = new File(new ReadFromPropertiesFile().getUploadPath() + File.separator + session.getAttribute("userId") + File.separator + session.getAttribute("path") + File.separator + new FolderServices().getFileName(Integer.parseInt(files[k]), Integer.parseInt(session.getAttribute("userId").toString())) + File.separator);
                f.delete();
            }
