...
public class FolderServices {
    ...
    public List<Integer> countFolderSize(String path) {
        int sum = 0;
        File f = new File(path);
        File[] farry = f.listFiles();
        for (File f1 : farry) {
            if (f1.isDirectory()) {
                countFolderSize(f1.getAbsolutePath());
            }
            if (f1.isFile()) {
                sum += f1.length();
            }
...
//        f.delete();
    ...
    public String getFileName(int fileId, int userId) {
        try (Connection conn = ConnectDB.connect();) {
            PreparedStatement pstmt = conn.prepareStatement("select file_name from filemaster where user_id=? and file_id=?");
            pstmt.setInt(1, userId);
            ...
            System.out.println("Exception in getFileName" + e);
        }
        return "Nothing";
    }
