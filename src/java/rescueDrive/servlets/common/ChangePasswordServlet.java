    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            if (request.getParameter("sbtnChange") != null) {
                String oldPass = request.getParameter("txtCurrentPassword");
                ...
                String result = objServices.changePassword(oldPass, newPass, Integer.parseInt(userId), type);
                if (type.equalsIgnoreCase("employee")) {
                    type = "administrator";
                }
                switch (type) {
                    case "administrator":
                        response.sendRedirect("admin/ChangePassword.jsp?msg=" + result);
                        break;
                    case "user":
                        response.sendRedirect("user/ChangePassword.jsp?msg=" + result);
                        break;
                }
            }
